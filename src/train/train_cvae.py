...
import argparse
from src.config import Pretrained1200_PATH, Pretrained1000_PATH
...
#    torch.distributed.init_process_group(backend='nccl')    
    ...
    if not (parameters["resume1"] == False):
        checkpointpath = Pretrained1200_PATH
        pretrained_dict = torch.load(checkpointpath, map_location=model.device)
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict)    

    if not (parameters["resume2"] == False):
        checkpointpath = Pretrained1000_PATH
        pretrained_dict = torch.load(checkpointpath, map_location=model.device)
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        model_dict.update(pretrained_dict)
