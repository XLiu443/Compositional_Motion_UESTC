...
#        start = time.time()
        ...
        if len(batch_video1_scores)==0:
            ...
            _, reconstruction1, video1_mask = self.mae_model(batch_resize_smpl, mask_ratio=0.5, score=batch_video1_scores.detach().cpu().numpy())
            _, reconstruction2, video2_mask = self.mae_model(batch_resize_smpl, mask_ratio=0.5, score=batch_video2_scores.detach().cpu().numpy())        
    ...
    def attention_map(self,skeleton_pointclouds,y1,y2,x1,x2,label):        
        ...
        score = self.human12_actionmask[convert_label[label]]
